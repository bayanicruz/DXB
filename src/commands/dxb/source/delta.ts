...
export default class extends SfdxCommand {
  ...
  protected static flagsConfig = {
    ...
    filter: flags.string({ char: 'f', description: 'diff filter', default: 'AMR' })
  };
  ...
  public getDeltaChanges(mode: any, deltakey: any, filter: any): any {
    var gitresult;
    if (mode === 'branch') {
      gitresult = exec(`git diff ${deltakey} --name-only --diff-filter=${filter}`).toString().split('\n');
    } else if (mode === 'tags') {
      if (deltakey) {
        gitresult = exec(`git diff $(git describe --match ${deltakey}* --abbrev=0 --all)..HEAD --name-only --diff-filter=${filter}`).toString().split('\n');
      } else {
        gitresult = exec(`git diff $(git describe --tags --abbrev=0 --all)..HEAD --name-only --diff-filter=${filter}`).toString().split('\n');
      }
    } else {
      gitresult = exec(`git diff-tree --no-commit-id --name-only -r ${deltakey} --diff-filter=${filter}`).toString().split('\n'); //this only work with specific commit ids, how to get file that changed since last tag ? 
    }
